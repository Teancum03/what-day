...
const links = [
  ...
  { name: 'Tuesday', to: '/tuesday' },
  { name: 'Wednesday', to: '/wednesday' },
  { name: 'Thursday', to: '/thursday' },
  { name: 'Friday', to: '/friday' },
]
      ...
      <div className="mx-auto max-w-7xl sm:px-2">
        <div className="flex items-center justify-between py-4  lg:space-x-2">
          ...
          <div className="hidden items-center md:flex">
            <Link
              to="/"
              className="rainbow-border rounded-md px-1 py-2 text-white hover:text-slate-700"
            >
              <div className="whitespace-nowrap">
                <FontAwesomeIcon
                  ...
                  style={{ color: '#FFFF00' }}
                />
                <span className="font-title text-2xl font-bold lg:text-3xl">
                  {'  What day is it?  '}
                </span>
                <FontAwesomeIcon
                  icon={faCalendar}
          ...
          {/* DESKTOP */}
          <ul className="hidden flex-1 justify-center md:flex xl:space-x-1">
            {links.map((link) => (
              <li key={link.to}>
          ...
          <div className="hidden md:flex md:flex-col">
            <span className="text-xs text-purple lg:text-lg lg:leading-none">
              Welcome,
            </span>
            {user ? (
              <DisplayAuthor name={user.longName} />
            ) : (
              <DisplayAuthor name={'Unknown White Umbrellabird'} />
            )}
          </div>
        </div>
      </div>
...
export function DesktopLink({
  ...
  return (
    <Link
      to={to}
      className="rounded-md px-2 py-2 font-header text-base font-bold text-slate-600 mix-blend-luminosity transition-colors hover:bg-slate-700/10 hover:text-slate-700 lg:px-4 lg:text-xl"
    >
      {children}
    </Link>
  )
